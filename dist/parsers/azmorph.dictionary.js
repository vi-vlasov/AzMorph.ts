...
export class DictionaryParse extends Parse {
    ...
    inflect(target, grammemes) {
        if (typeof target === 'number') {
            if (target < 0 || target >= this.formCnt)
                return false;
            return new DictionaryParse(prefixes[this.paradigm[(this.formCnt << 1) + target]] +
                this.base() +
                suffixes[this.paradigm[target]], this.paradigmIdx, target, 0, 0, this.prefix, this.suffix, this.tags);
        }
        for (let formIdx = 0; formIdx < this.formCnt; formIdx++) {
            ...
            if (formTag.matches(target, grammemes)) {
                return new DictionaryParse(prefixes[this.paradigm[(this.formCnt << 1) + formIdx]] +
                    this.base() +
                    suffixes[this.paradigm[formIdx]], this.paradigmIdx, formIdx, 0, 0, this.prefix, this.suffix, this.tags);
            }
        }
        return false;
    }
