...
class Tokens {
   ...
   */
    append(text, config) {
        ...
        config = config ? AzMorphLoader.extend(this.config, config) : this.config;
        ...
        for (let i = offs; i < s.length; i++) {
            ...
            if (token) {
                ...
                if (token.type === Tokens.LINK) {
                ...
                else if ((token.type === Tokens.HASHTAG) || (token.type === Tokens.MENTION)) {
                ...
                else if ((token.type === Tokens.TAG) && (token.quote || (s[token.en()] != '>'))) {
                    append = true;
                    if (token.quote) {
                    ...
                    if (token.quote) {
                        ...
                        else if ((token.length >= 7) && (token.toString().substr(-7) == '</style')) {
                            token.length -= 7;
                            st -= 7;
                            append = false;
                            tokenType = Tokens.TAG;
                ...
                else if ((token.type === Tokens.MARKUP) &&
                    ...
                    else if (token.type === Tokens.NUMBER) { // Digits + ending
                        append = true;
                        token.subType = (token.subType && token.subType != charType) ? Tokens.MIXED : charType;
                    }
                    else if (config.hashtags && (token.length == 1) && (s[token.st] == '#')) { // Hashtags
                        append = true;
                        token.type = Tokens.HASHTAG;
                    }
                    else if (config.mentions &&
                        (token.length == 1) &&
                        (s[token.st] == '@') &&
                    ...
                    else if ((token.length == 1) && (s[token.st] == '-')) { // -цать (?), 3-й
                        append = true;
