...
let words = {}; // TODO: сюда загрузить список слов 
let initialized = false;
export const Morph = (word, config = defaults) => {
    if (!initialized) {
...
function normalizeScores(parses) {
    const groups = {
...
__init.push(function () {
    Morph.Parsers.Dictionary = function (word, config) {
        let isCapitalized = !config.ignoreCase && word.length &&
        ...
        for (let i = 0; i < opts.length; i++) {
            for (let j = 0; j < opts[i][1].length; j++) {
                let w = new DictionaryParse(opts[i][0], opts[i][1][j][0], opts[i][1][j][1], opts[i][2], opts[i][3], undefined, undefined, tags);
    ...
    for (let i = 0; i <= 2; i++) {
        for (let j = 0; j <= 5; j++) {
            for (let k = 0; k <= 1; k++) {
                abbrTags.push(makeTag('NOUN,inan,' + ['masc', 'femn', 'neut'][i] + ',Fixd,Abbr ' + ['sing', 'plur'][k] + ',' + ['nomn', 'gent', 'datv', 'accs', 'ablt', 'loct'][j], 'СУЩ,неод,' + ['мр', 'жр', 'ср'][i] + ',0,аббр ' + ['ед', 'мн'][k] + ',' + ['им', 'рд', 'дт', 'вн', 'тв', 'пр'][j]));
            }
        }
    }
    ...
    let InitialsParser = function (score) {
        let initialsTags = [];
        for (let i = 0; i <= 1; i++) {
            for (let j = 0; j <= 5; j++) {
                initialsTags.push(makeTag('NOUN,anim,' + ['masc', 'femn'][i] + ',Sgtm,Name,Fixd,Abbr,Init sing,' + ['nomn', 'gent', 'datv', 'accs', 'ablt', 'loct'][j], 'СУЩ,од,' + ['мр', 'жр'][i] + ',sg,имя,0,аббр,иниц ед,' + ['им', 'рд', 'дт', 'вн', 'тв', 'пр'][j]));
            }
        }
    ...
    Morph.Parsers.IntNumber = RegexpParser(/^[−-]?[0-9]+$/, makeTag('NUMB,intg', 'ЧИСЛО,цел'), 0.9);
    Morph.Parsers.RealNumber = RegexpParser(/^[−-]?([0-9]*[.,][0-9]+)$/, makeTag('NUMB,real', 'ЧИСЛО,вещ'), 0.9);
    Morph.Parsers.Punctuation = RegexpParser(/^[\u2000-\u206F\u2E00-\u2E7F\\'!"#$%&()*+,\-.\/:;<=>?@\[\]^_`{|}~]+$/, makeTag('PNCT', 'ЗПР'), 0.9);
    Morph.Parsers.RomanNumber = RegexpParser(/^M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/, makeTag('ROMN', 'РИМ'), 0.9);
    Morph.Parsers.Latin = RegexpParser(/[A-Za-z\u00C0-\u00D6\u00D8-\u00f6\u00f8-\u024f]$/, makeTag('LATN', 'ЛАТ'), 0.9);
    // слово + частица
    // смотри-ка
    Morph.Parsers.HyphenParticle = function (word, config) {
        ...
        for (let k = 0; k < particles.length; k++) {
            if (word.substr(word.length - particles[k].length) == particles[k]) {
                ...
                for (let i = 0; i < opts.length; i++) {
                    for (let j = 0; j < opts[i][1].length; j++) {
                        let w = new DictionaryParse(opts[i][0], opts[i][1][j][0], opts[i][1][j][1], opts[i][2], opts[i][3], '', particles[k], tags);
                        w.score *= 0.9;
                        lets.push(w);
                    }
                }
    ...
    Morph.Parsers.HyphenAdverb = function (word, config) {
        ...
        for (let i = 0; i < opts.length; i++) {
            if (!used[opts[i][0]]) {
                for (let j = 0; j < opts[i][1].length; j++) {
                    let parse = new DictionaryParse(opts[i][0], opts[i][1][j][0], opts[i][1][j][1], opts[i][2], opts[i][3], tags);
            ...
            for (let len = 5; len >= minlen; len--) {
                ...
                for (let j = 0; j < entries.length; j++) {
                    ...
                    for (let k = 0; k < stats.length; k++) {
                        let parse = new DictionaryParse(prefixes[i] + left + suffix, stats[k][1], stats[k][2]);
 ...
 *  всех словарей.
 */
export const Init = async (path = 'dicts') => {
    const [wordsDawg, suffixDawg0, suffixDawg1, suffixDawg2, probabilitiesDawg, tagsIntJson, tagsExtJson, suffixesJson, paradigmsBuffer] = await Promise.all([
    ...
    words = Dawg.fromArrayBuffer(wordsDawg, 'words');
    predictionSuffixes[0] = Dawg.fromArrayBuffer(suffixDawg0, 'probs');
    predictionSuffixes[1] = Dawg.fromArrayBuffer(suffixDawg1, 'probs');
    predictionSuffixes[2] = Dawg.fromArrayBuffer(suffixDawg2, 'probs');
    probabilities = Dawg.fromArrayBuffer(probabilitiesDawg, 'probs');
    setSuffixes(suffixesJson);
    tags = tagsIntJson.map((t, i) => {
        const tag = new Tag(t);
        tag.ext = new Tag(tagsExtJson[i]);
        return tag;
    });
    Object.freeze(tags);
    {
        const list = new Uint16Array(paradigmsBuffer);
        const count = list[0];
        let pos = 1;
        setParadigms([]);
